...
class ControladorDinamico:
    ...
    def auditar_posiciones(self, symbol="AAVEUSDT", price_precision=3):
        ...
        try:
            ...
            posiciones = self.client.get_position_risk(symbol=symbol, timestamp=ts)
            ...
            for pos in posiciones:
                ...
                # 1. Lógica de Break Even (Asegurar capital + ganancia mínima)
                if pnl_pct >= self.be_activation_pct:
                    ...
                    # Marcar la orden como "protegida" en el Gestor de Cupos para liberar riesgo
                    self._marcar_orden_protegida(entry_price)

                # 2. Lógica de Trailing Stop (Perseguir el precio)
                if pnl_pct > (self.be_activation_pct + 0.01):
                    # El precio ha avanzado más allá del BE. Empezamos a seguirlo.
            ...
            print(f"⚠️ [Control] Error auditando posiciones: {e.error_message}")
    ...
    def _actualizar_stop_loss(self, symbol, side_posicion, cantidad, nuevo_precio_sl, precision, tipo):
        ...
        try:
            ...
            # 1. Cancelar órdenes de Stop actuales para esa posición (para no duplicar)
            ordenes_abiertas = self.client.get_open_orders(symbol=symbol, timestamp=ts)
            for orden in ordenes_abiertas:
                if orden['type'] == 'STOP_MARKET' and orden['positionSide'] == position_side:
                    self.client.cancel_order(symbol=symbol, orderId=orden['orderId'], timestamp=ts)
            
            # 2. Colocar el nuevo Stop Loss Ajustado
            params_sl = {
                "symbol": symbol,
                "side": side_orden,
    ...
    def _marcar_orden_protegida(self, entry_price):
        for orden in self.gestor_cupos.posiciones_activas:
            # Validamos por precio de entrada (con pequeña tolerancia por redondeo)
